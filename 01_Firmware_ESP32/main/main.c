 ...
 * Update: SEPARATE LED CURRENTS (Red/IR) + UART BAUD RATE CONTROL
 */

#include "driver/i2c.h"
#include "driver/uart.h" // ADDED: Required for changing Baud Rate
#include "esp_log.h"
...
#define LED_CURRENT_RED_MA                                                     \
  10.0 // Red often needs less power (avoid skin saturation)
...
static void max30102_init_sensor(void) {
  ...
  uint8_t spo2_conf =
      (ADC_RANGE_OPT << 5) | (SAMPLE_RATE_OPT << 2) | (PULSE_WIDTH_OPT);
  ...
  ESP_LOGI(TAG, "- LED RED Current: %.1f mA (Reg: 0x%02X)", LED_CURRENT_RED_MA,
           led_pa_red);
  ESP_LOGI(TAG, "- LED IR  Current: %.1f mA (Reg: 0x%02X)", LED_CURRENT_IR_MA,
           led_pa_ir);
    ...
    if (max30102_read_reg(0x04, ptr_data, 3) == ESP_OK) {
      ...
      while (samples_to_read > 0) {
        if (max30102_read_reg(0x07, fifo_data, 6) == ESP_OK) {
          ...
          uint32_t red_val =
              ((fifo_data[0] << 16) | (fifo_data[1] << 8) | fifo_data[2]) &
              0x03FFFF;
          uint32_t ir_val =
              ((fifo_data[3] << 16) | (fifo_data[4] << 8) | fifo_data[5]) &
              0x03FFFF;

          if (ir_val < FINGER_THRESHOLD) {
            printf("0,0\n");
          } else {
            // Keep strict format: IR, RED
            printf("%u,%u\n", ir_val, red_val);
          }
        }
        samples_to_read--;
...
void app_main(void) {
  ...
  // 2. Configure UART Baud Rate (ADDED THIS STEP)
  //    Sets the console UART (UART_NUM_0) to the user-defined speed.
